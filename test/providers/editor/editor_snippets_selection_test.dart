...
void main() {
  ...
  group('EditorNotifier — snippet palettes', () {
    ...
    test('setSnippetPaletteThreadColor produces new snippetEditorState.palettes instance', () {
      n(c).initSnippetPalettesLocal(
          [SnippetPalette.create(name: 'P1', threads: [_black])], 0);
      final before = s(c).snippetEditorState.palettes;
      n(c).setSnippetPaletteThreadColor(0, 0, _red);
      expect(identical(s(c).snippetEditorState.palettes, before), isFalse,
  ...
  group('EditorNotifier — saveSelectionAsSnippet', () {
    ...
    test('saveSelectionAsSnippet with no selection → returns false + warning', () {
      // No editSession.selectionRect set.
      final ok = n(c).saveSelectionAsSnippet('Empty');
      expect(ok, isFalse);
      expect(s(c).editSession.pendingCanvasWarning, isNotNull);
    });
  ...
  // ─── Selection & editSession.clipboard ────────────────────────────────────────────────────
  ...
  group('EditorNotifier — selection', () {
    ...
    test('setSelectionRect stores rect', () {
      ...
      expect(s(c).editSession.selectionRect, equals(const Rect.fromLTRB(1, 2, 5, 6)));
    });

    test('selectAll sets rect to full canvas', () {
      n(c).selectAll();
      final r = s(c).editSession.selectionRect!;
      ...
      expect(s(c).editSession.drawingMode, equals(DrawingMode.select));
    });

    test('cancelSelection clears rect (stays in select mode; draw mode not changed)', () {
      n(c).selectAll();
      expect(s(c).editSession.selectionRect, isNotNull);
      n(c).cancelSelection();
      expect(s(c).editSession.selectionRect, isNull);
  ...
  group('EditorNotifier — copy/paste', () {
    ...
    test('copySelection populates in-memory editSession.clipboard and enters paste mode', () async {
      ...
      expect(s(c).editSession.clipboard, isNotNull);
      expect(s(c).editSession.clipboard, isNotEmpty);
      expect(s(c).editSession.drawingMode, equals(DrawingMode.paste));
      expect(s(c).editSession.selectionRect, isNull); // cleared on copy
    });

    test('copySelection on empty selection → warning, no editSession.clipboard change', () async {
      n(c).setSelectionRect(const Rect.fromLTRB(5, 5, 10, 10)); // no stitches there
      final prevClip = s(c).editSession.clipboard;
      await n(c).copySelection();
      expect(s(c).editSession.clipboard, equals(prevClip)); // unchanged
      expect(s(c).editSession.pendingCanvasWarning, isNotNull);
    });

    test('commitPaste stamps editSession.clipboard at offset onto active layer', () async {
    ...
    test('paste auto-adds editSession.clipboard threads not in pattern', () async {
      ...
      n(c).setMode(AppMode.edit);
      // Re-enter paste mode using the preserved editSession.clipboard.
    ...
    test('copySelection with no editSession.selectionRect → warning', () async {
      // No rect set at all.
      await n(c).copySelection();
      expect(s(c).editSession.pendingCanvasWarning, isNotNull);
    });
  ...
  group('EditorNotifier — loadSnippetToClipboard', () {
    ...
    test('switches to edit mode when called from view mode', () async {
      ...
      expect(s(c).editSession.drawingMode, DrawingMode.paste,
    ...
    test('editSession.clipboardFromSnippet is true after loadSnippetToClipboard', () async {
      final snippet = _makeSnippet();
      final p = s(c).pattern.copyWith(snippets: [snippet]);
      n(c).loadPattern(p,
          session: EditorSession(selectedThreadId: p.editorSelectedThreadId));
      await n(c).loadSnippetToClipboard(snippet);
      expect(s(c).editSession.clipboardFromSnippet, isTrue);
    });
  });
