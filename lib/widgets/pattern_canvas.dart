...
class _PatternCanvasState extends ConsumerState<PatternCanvas> {
  ...
  Offset _canvasToGridPoint(Offset canvas) {
    // Snap to 0.5-cell increments for backstitch
  ...
  /// Returns the sub-cell position (0.0–1.0 within cell) for the given canvas point.
  ...
  HalfOrientation _detectHalf(double subX, double subY) {
    // Choose horizontal split if the tap is more to the left/right of center,
    // or vertical split if more above/below center.
  ...
  Stitch? _buildStitch(DrawingTool tool, int x, int y, String threadId,
    ...
    return switch (tool) {
      ...
      DrawingTool.quarterCross => QuarterCrossStitch(
          ...
          quadrant: _detectQuadrant(subX, subY),
          threadId: threadId),
      DrawingTool.backstitch || DrawingTool.navigate => null,
    };
  }
  ...
  void _onPointerDown(PointerDownEvent event) {
    ...
    if (event.kind == PointerDeviceKind.stylus ||
      ...
      // Apple Pencil double-tap → kSecondaryStylusButton
      if (event.kind == PointerDeviceKind.stylus &&
  ...
  void _onPointerMove(PointerMoveEvent event) {
    ...
    if (event.kind == PointerDeviceKind.stylus ||
      ...
      // Update backstitch hover preview
      if (state.currentTool == DrawingTool.backstitch) {
    ...
    if (_activePointers.length == 2) {
      ...
      if (_pinchStartDistance > 0) {
        final newScale = (_gestureStartScale * currentDist / _pinchStartDistance)
            .clamp(0.1, 20.0);
      ...
      if (state.currentTool == DrawingTool.navigate) {
        setState(() {
  ...
  void _onPointerSignal(PointerSignalEvent event) {
    if (event is PointerScrollEvent) {
      ...
      setState(() {
        final newScale = (_scale * zoomFactor).clamp(0.1, 20.0);
