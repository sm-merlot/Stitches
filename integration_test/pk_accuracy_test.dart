...
void main() {
  ...
  group('PK PDF round-trip accuracy (sm_test)', () {
    test('sm_test.stitches → PK PDF → parse → identical canvas', () async {
      ...
      debugPrint('PDF: ${(pdfBytes.length / 1024).toStringAsFixed(1)} KB');
      ...
      try {
        ...
        // 7. Build parsed map — order-independent, no GUIDs involved.
        ...
        // Thread sets (by DMC code — no IDs).
        ...
        // 9. Detailed report (always printed so CI logs show context on failure).
        debugPrint('');
        debugPrint('══════════════════════════════════════════');
